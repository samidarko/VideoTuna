...
from videotuna.base.utils_diffusion import make_beta_schedule, rescale_zero_terminal_snr, discretized_gaussian_log_likelihood
...
class IDDPM(DDPMFlow):
    ...
    def _vb_terms_bpd(self, x_start, x_t, t, clip_denoised=True, model_kwargs=None, mask=None):
        """
...
class LatentDiffusion(SpacedDiffusion):
    ...
    def p_losses(self, x_start, cond, t, noise=None, model_kwargs=None, mask=None, weights=None, **kwargs):
        if model_kwargs is None:
            model_kwargs = {}
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    def _vb_terms_bpd(self, x_start, x_t, c, t, clip_denoised=True, model_kwargs=None, mask=None, **kwargs):
        """
        Get a term for the variational lower-bound.
        The resulting units are bits (rather than nats, as one might expect).
        This allows for comparison to other papers.
