...
def convert_lora(model, state_dict, LORA_PREFIX_UNET="lora_unet", LORA_PREFIX_TEXT_ENCODER="lora_te", alpha=0.6):
    ...
    for key in state_dict:
        ...
        if "text" in key and LORA_PREFIX_TEXT_ENCODER in key:
            ...
            layer_infos = ["cond_stage_model", "transformer"] + key.split(".")[0].split(LORA_PREFIX_TEXT_ENCODER + "_")[-1].split("_") 
            curr_layer = model
...
def change_sd_weight(model, sd_sd,
    ...
    for k, v in sd_sd.items():
        ...
        if 'diffusion_model.middle_block.2.' in k:
            k = k.replace('middle_block.2', 'middle_block.3') # 2 is temporal transformer, 3 is resblock
        ...
        # merge new token
        if time_varied_inversion_v2 and k == "cond_stage_model.model.token_embedding.weight":
            n_token=sd_sd["cond_stage_model.model.token_embedding.weight"].shape[0]
            v = torch.cat([sd_sd[k].to(device), 
                           model_sd[k][n_token:]],
                           dim=0)
            # v = model_sd[k]
        model_sd[k] = v
...
def load_model_checkpoint_t2v(model, ckpt,
                          lora_path=None, lora_scale=1.0, 
                          lora_path_2=None, lora_scale_2=1.0,
                          time_varied_inversion_v2=False,
                          strict=True,
                          sd_ckpt=None,
                          ):
    ...
    if merge_new_token:
        if lora_path is not None:
            ...
            if sd_lora_and_adapter_lora: # tmp code.  combine a sd lora with a adapter lora
                token_emb = None
            else:
                token_emb = lora_sd["cond_stage_model.model.token_embedding.weight"]
            pl_sd["cond_stage_model.token_table.weight"] = lora_sd["cond_stage_model.token_table.weight"]
            del lora_sd
    ...
    if sd_ckpt is not None:
        ...
        model = change_sd_weight(model, sd_sd, 
    ...
    if lora_path is not None:
        ...
        if lora_path_2 is not None:
            net_load_lora(model, lora_path_2, alpha=lora_scale_2,
                          load_text_encoder=load_lora_text_encoder,
                          )
