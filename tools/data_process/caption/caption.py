...
def get_inf1(folder_path):
    ...
    with open(file_path+'/'+os.path.splitext(os.path.basename(folder_path))[0]+'.txt', 'r') as info:
...
def inference(args):
    ...
    video_list=[data for data in video_list if not os.path.exists(save_path+'/'+os.path.splitext(os.path.basename(data))[0]+'.json')]
    ...
    for video in tqdm.tqdm(video_list):
        try:
            ...
            with open(args.out_dir+'/'+os.path.splitext(os.path.basename(video_path))[0]+".json",'w') as f:
...
if __name__=='__main__':
    ...
    parser.add_argument('--model_path',type=str,default="/project/llmsvgen/pengjun/LLaVA-NeXT/llava-onevision-qwen2-7b-ov")
    parser.add_argument('--vid_dir',type=str,default="/project/llmsvgen/share/data_trailer_human/videos")
    parser.add_argument('--out_dir',type=str,default="/project/llmsvgen/pengjun/LLaVA-NeXT2/trailer_caption")
