...
def main():
    ...
    if prompts is None:
        if cfg.get("prompt_path", None) is not None:
            prompts_dict = load_prompts(cfg.prompt_path)
        else:
    ...
    for key, prompts in prompts_dict.items():
            ...
            for k in range(num_sample):
                ...
                if is_main_process():
                    for idx, batch_prompt in enumerate(batch_prompts):
                        ...
                        if k == 0:
                            format_file[save_path.split("/")[-1]] = batch_prompt
        ...
        for k in range(num_sample):
            with open(Path(save_dir) / key / f'{k}.json', 'w') as f:
