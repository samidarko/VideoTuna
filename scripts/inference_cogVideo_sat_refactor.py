    ...
    with torch.no_grad():
        for text, cnt in tqdm(data_iter):
            if args.image2video:
                ...
                H, W = (96, nearest_multiple_of_16(img_W / img_H * 96 * 8) // 8) if img_H < img_W else \
                       (nearest_multiple_of_16(img_H / img_W * 96 * 8) // 8, 96)
                ...
                image = torch.cat([image, torch.zeros(pad_shape, device=image.device, dtype=image.dtype)], dim=1)
            else:
            ...
            # )
            conditioner_keys = list(set([x.input_key for x in model.conditioner.embedders]))
            ...
            for key in c:
                if key != "crossattn":
                    c[key], uc[key] = map(lambda y: y[key][:math.prod(num_samples)].to("cuda"), (c, uc))
            if args.image2video:
            ...
            for index in range(args.batch_size):
                ...
                if args.only_save_latents:
                    torch.save(samples_z / model.scale_factor, os.path.join(save_path, "latent.pt"))
